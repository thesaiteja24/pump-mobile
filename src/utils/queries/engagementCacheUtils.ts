...
export function updateInfiniteData<
  TPage extends { [K in TKey]: unknown[] },
  TKey extends keyof TPage,
>(
 ...
 */
export function prependToInfiniteData<
  TPage extends { [K in TKey]: { id: string }[] },
  TKey extends keyof TPage,
>(
  oldData: InfiniteData<TPage> | undefined,
  itemKey: TKey,
...
export function updateCommentAcrossCaches(
  ...
  const queryKey = workoutId
    ? queryKeys.engagement.comments(workoutId)
    : queryKeys.engagement.commentsRoot
  qc.setQueriesData({ queryKey }, (old: InfiniteData<CommentsPage> | undefined) =>
    updateInfiniteData(old, 'comments', (items) =>
      items.map((c) => (c.id === commentId ? updater(c) : c)),
    ),
  )
  ...
  qc.setQueriesData(
    ...
    (old: InfiniteData<RepliesPage> | undefined) =>
      updateInfiniteData(old, 'replies', (items) =>
        items.map((r) => (r.id === commentId ? updater(r) : r)),
      ),
  )
}
  ...
  qc.setQueriesData(
    ...
    (old: InfiniteData<CommentsPage> | undefined) =>
      updateInfiniteData(old, 'comments', (items) =>
        items.map((c) =>
          c.id === id
            ? { ...c, isLiked: liked, likesCount: Math.max(0, c.likesCount + (liked ? 1 : -1)) }
            : c,
        ),
      ),
  )
  ...
  qc.setQueriesData(
    ...
    (old: InfiniteData<RepliesPage> | undefined) =>
      updateInfiniteData(old, 'replies', (items) =>
        items.map((r) =>
          r.id === id
            ? { ...r, isLiked: liked, likesCount: Math.max(0, r.likesCount + (liked ? 1 : -1)) }
            : r,
        ),
      ),
  )
...
export function toggleLikeInWorkouts(qc: QueryClient, id: string, liked: boolean) {
  ...
  qc.setQueriesData(
    ...
    (old: InfiniteData<{ workouts: any[] }> | undefined) =>
      updateInfiniteData(old, 'workouts', (items) =>
        items.map((w: any) =>
          w.id === id
            ? { ...w, isLiked: liked, likesCount: Math.max(0, w.likesCount + (liked ? 1 : -1)) }
            : w,
        ),
      ),
  )
